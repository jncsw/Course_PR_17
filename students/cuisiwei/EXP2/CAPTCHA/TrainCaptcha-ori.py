...
if __name__ == '__main__':
    ...
    with tf.name_scope('fc1'):
        ...
        W_fc1 = tf.Variable(tf.truncated_normal([int(math.ceil(CaptchaAttr['imgWidth']/2.0/2.0)) * int(math.ceil(CaptchaAttr['imgHeight']/2.0/2.0)) * 64, 1024],stddev=0.1));

        tf.summary.histogram(layer_name + "/biases", W_fc1)  # 可视化观看变量
        b_fc1 = tf.Variable(tf.constant(0.1,shape=[1024]));
        h_pool2_flat = tf.reshape(h_pool2,[-1,int(math.ceil(CaptchaAttr['imgWidth']/2.0/2.0)) * int(math.ceil(CaptchaAttr['imgHeight']/2.0/2.0)) * 64]);
